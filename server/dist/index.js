...
const epoNoten_1 = __importDefault(require("./routes/epoNoten"));
...
function requestHostPort(hostHeader) {
    const raw = hostHeader.trim().toLowerCase();
    if (raw.startsWith('[')) {
        const end = raw.indexOf(']');
        if (end >= 0 && raw[end + 1] === ':')
            return raw.slice(end + 2);
        return '';
    }
...
app.use('/api/epo-noten', epoNoten_1.default);

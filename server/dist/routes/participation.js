...
router.get('/student/:studentId', async (req, res) => {
    try {
        ...
        Object.keys(groupParticipations).forEach(groupId => {
            ...
            if (structured[groupId].average >= 1.5)
                structured[groupId].grade = 1.0;
            else if (structured[groupId].average >= 0.5)
                structured[groupId].grade = 2.0;
            else if (structured[groupId].average >= -0.5)
                structured[groupId].grade = 3.0;
            else if (structured[groupId].average >= -1.5)
                structured[groupId].grade = 4.0;
            else
                structured[groupId].grade = 5.0;
        });
        res.json(structured);
    }
    catch (error) {
...
router.get('/:groupId/periods', async (req, res) => {
    try {
        const { groupId } = req.params;
        const group = await prisma.learningGroup.findUnique({
            where: { id: groupId },
...
router.put('/:groupId/periods', async (req, res) => {
    try {
        const { groupId } = req.params;
...
router.post('/:groupId/calculate-epo', async (req, res) => {
    try {
        const { groupId } = req.params;
        const group = await prisma.learningGroup.findUnique({
            where: { id: groupId },
    ...
    try {
        ...
        Object.keys(studentStats).forEach(studentId => {
            ...
            if (stats.period1.count > 0) {
                ...
                if (stats.period1.average >= 1.5)
                    stats.period1.grade = 1.0;
                else if (stats.period1.average >= 0.5)
                    stats.period1.grade = 2.0;
                else if (stats.period1.average >= -0.5)
                    stats.period1.grade = 3.0;
                else if (stats.period1.average >= -1.5)
                    stats.period1.grade = 4.0;
                else
                    stats.period1.grade = 5.0;
            }
            // Berechne Period 2
            if (stats.period2.count > 0) {
                stats.period2.average = stats.period2.average / stats.period2.count;
                if (stats.period2.average >= 1.5)
                    stats.period2.grade = 1.0;
                else if (stats.period2.average >= 0.5)
                    stats.period2.grade = 2.0;
                else if (stats.period2.average >= -0.5)
                    stats.period2.grade = 3.0;
                else if (stats.period2.average >= -1.5)
                    stats.period2.grade = 4.0;
                else
                    stats.period2.grade = 5.0;
            }
        });
...
router.put('/:groupId/:lessonIndex/:studentId/comment', async (req, res) => {
    try {
        ...
        if (isReleased) {
            ...
            try {
                ...
                if (schema) {
                    ...
                    const gradesToDelete = allGrades.filter(g => g.categoryName.trim().toLowerCase() === categoryName.toLowerCase());
...
router.get('/:groupId/epo-grades', async (req, res) => {
    try {
        const { groupId } = req.params;
        const epoGrades = await prisma.participationPeriodGrade.findMany({
            where: { groupId },

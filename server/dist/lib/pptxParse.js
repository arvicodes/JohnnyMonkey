...
exports.parsePptxBuffer = parsePptxBuffer;
...
function emuToPct(rect, slideCx, slideCy) {
    return {
...
function parseAttrNumber(tag, name) {
...
function parseGroupXfrm(grpSpPrXml) {
    var _a, _b, _c, _d, _e, _f, _g, _h;
    const offTag = (_a = grpSpPrXml.match(/<a:off\b[^>]*\/?>/i)) === null || _a === void 0 ? void 0 : _a[0];
    const extTag = (_b = grpSpPrXml.match(/<a:ext\b[^>]*\/?>/i)) === null || _b === void 0 ? void 0 : _b[0];
    const chOffTag = (_c = grpSpPrXml.match(/<a:chOff\b[^>]*\/?>/i)) === null || _c === void 0 ? void 0 : _c[0];
    const chExtTag = (_d = grpSpPrXml.match(/<a:chExt\b[^>]*\/?>/i)) === null || _d === void 0 ? void 0 : _d[0];
    if (!offTag || !extTag)
        return null;
    ...
    if (ox == null || oy == null || ecx == null || ecy == null)
        return null;
    return {
        off: { x: ox, y: oy },
        ext: { cx: ecx, cy: ecy },
        chOff: {
            x: chOffTag ? (_e = parseAttrNumber(chOffTag, 'x')) !== null && _e !== void 0 ? _e : 0 : 0,
            y: chOffTag ? (_f = parseAttrNumber(chOffTag, 'y')) !== null && _f !== void 0 ? _f : 0 : 0,
        },
        chExt: {
            cx: chExtTag ? (_g = parseAttrNumber(chExtTag, 'cx')) !== null && _g !== void 0 ? _g : ecx : ecx,
            cy: chExtTag ? (_h = parseAttrNumber(chExtTag, 'cy')) !== null && _h !== void 0 ? _h : ecy : ecy,
        },
    };
}
function applyGroupTransform(local, group) {
    if (!group)
        return local;
...
function applyColorTransforms(hex, transformXml) {
...
function loadThemeColors(zip) {
    ...
    const clrScheme = ((_a = xml.match(/<a:clrScheme[\s>][\s\S]*?<\/a:clrScheme>/i)) === null || _a === void 0 ? void 0 : _a[0]) || '';
    for (const m of clrScheme.matchAll(/<a:([a-z0-9]+)>([\s\S]*?)<\/a:\1>/gi)) {
        ...
        const hex = (srgb === null || srgb === void 0 ? void 0 : srgb[1]) || (sys === null || sys === void 0 ? void 0 : sys[1]);
        if (hex)
...
function extractSrgb(xml, theme) {
    ...
    if (srgb) {
        ...
        if (val)
            return applyColorTransforms(`#${val.toUpperCase()}`, attrs + inner);
    }
...
function extractFillColor(spXml, theme) {
    var _a;
    if (/<a:noFill\s*\/>/i.test(((_a = spXml.match(/<p:spPr[\s>][\s\S]*?<\/p:spPr>/i)) === null || _a === void 0 ? void 0 : _a[0]) || '')) {
        return null;
    }
    const solid = spXml.match(/<a:solidFill>([\s\S]*?)<\/a:solidFill>/i);
    if (solid)
        return extractSrgb(solid[1], theme);
    return null;
}
function extractStrokeColor(spXml, theme) {
    const ln = spXml.match(/<a:ln[\s>][\s\S]*?<\/a:ln>/i);
    if (!ln)
        return null;
    if (/<a:noFill\s*\/>/i.test(ln[0]))
        return null;
    return extractSrgb(ln[0], theme);
}
function extractFontSizePt(spXml) {
    const sz = spXml.match(/<a:rPr[^>]*\bsz="(\d+)"/i) ||
...
function defaultFontPtForPlaceholder(phType) {
...
function paragraphsToHtml(txBodyXml, theme, fallbackFontPt) {
    const paras = [...txBodyXml.matchAll(/<a:p[\s>][\s\S]*?<\/a:p>/gi)].map((m) => m[0]);
    const plainLines = [];
    let anyBold = false;
    let color = null;
    let fontSizePt = fallbackFontPt;
    const htmlParts = [];
    const bodyDefSz = txBodyXml.match(/<a:defRPr[^>]*\bsz="(\d+)"/i);
    if (bodyDefSz && fontSizePt == null)
        fontSizePt = Number(bodyDefSz[1]) / 100;
    for (const p of paras) {
        ...
        for (const r of runs) {
            ...
            const attrs = (rPr === null || rPr === void 0 ? void 0 : rPr[1]) || '';
            ...
            if (runColor && !color)
                color = runColor;
            if (pt && (fontSizePt == null || pt > fontSizePt))
                fontSizePt = pt;
        ...
        const alignStyle = pAlign === 'ctr' || pAlign === 'center'
...
function parseRelTargets(relsXml) {
    const map = new Map();
    const re = /<Relationship[^>]*\bId="(rId\d+)"[^>]*\bTarget="([^"]+)"[^>]*\/?>|<Relationship[^>]*\bTarget="([^"]+)"[^>]*\bId="(rId\d+)"[^>]*\/?>/gi;
    let m;
...
function resolveZipEntry(zip, mediaTarget) {
    const cleaned = mediaTarget.replace(/^\.\.\//g, '').replace(/^\//, '');
    const candidates = [
        cleaned,
        path_1.default.posix.join('ppt', cleaned.replace(/^ppt\//, '')),
        path_1.default.posix.normalize(`ppt/slides/${mediaTarget}`),
        `ppt/media/${path_1.default.posix.basename(cleaned)}`,
    ];
    ...
    const base = path_1.default.posix.basename(cleaned).toLowerCase();
    for (const e of zip.getEntries()) {
        const n = e.entryName.replace(/\\/g, '/').toLowerCase();
        if (n.endsWith('/' + base) || n === base)
            return e;
    }
    return null;
}
function loadPlaceholdersFromXml(xml) {
    const out = [];
...
function resolveThemeOrLayoutPath(zip, relTarget, fromDir) {
    const cleaned = relTarget.replace(/\\/g, '/');
    const candidates = [
        cleaned.replace(/^\.\.\//, 'ppt/'),
        path_1.default.posix.normalize(path_1.default.posix.join(fromDir, cleaned)),
        `ppt/slideLayouts/${path_1.default.posix.basename(cleaned)}`,
        `ppt/slideMasters/${path_1.default.posix.basename(cleaned)}`,
    ];
    for (const c of candidates) {
...
function loadLayoutPlaceholders(zip, layoutRelTarget) {
    ...
    const layoutRels = zip.getEntry(layoutRelsPath) || zip.getEntry(layoutRelsPath.replace(/\//g, '\\'));
    if (layoutRels) {
        const relMap = parseRelTargets(layoutRels.getData().toString('utf8'));
        for (const [, target] of relMap) {
            if (!/slideMaster/i.test(target))
                continue;
...
function findPlaceholderRect(placeholders, phAttrs) {
    var _a, _b;
    ...
    if (type === 'ctrTitle' || type === 'title') {
        return ((_a = placeholders.find((p) => p.type === 'ctrTitle' || p.type === 'title')) === null || _a === void 0 ? void 0 : _a.rect) || null;
    }
    return ((_b = placeholders.find((p) => p.type === 'body' || p.type === 'obj')) === null || _b === void 0 ? void 0 : _b.rect) || null;
}
function shapeLooksLikeBox(_spXml, fill) {
    return Boolean(fill);
}
function extractSlideBackground(slideXml, theme) {
    var _a;
    const bg = (_a = slideXml.match(/<p:bg[\s>][\s\S]*?<\/p:bg>/i)) === null || _a === void 0 ? void 0 : _a[0];
    if (!bg)
        return null;
    if (/<a:noFill\s*\/>/i.test(bg))
        return null;
...
function tableToHtml(tblXml, theme) {
...
function extractShapeBoxes(xml, zip, relMap, placeholders, group, slideCx, slideCy, theme, boxes) {
    var _a, _b, _c, _d;
    for (const grp of extractTopLevelBlocks(xml, 'p:grpSp')) {
        const grpPr = ((_a = grp.match(/<p:grpSpPr[\s>][\s\S]*?<\/p:grpSpPr>/i)) === null || _a === void 0 ? void 0 : _a[0]) || '';
        ...
        const inner = grp
            ...
            .replace(/<\/p:grpSp>$/i, '');
        extractShapeBoxes(inner, zip, relMap, placeholders, combined || group, slideCx, slideCy, theme, boxes);
    }
    ...
    for (const pic of extractTopLevelBlocks(flat, 'p:pic')) {
        ...
        if (!media)
            continue;
        const ext = path_1.default.extname(media.entryName).toLowerCase();
        const mime = MIME_BY_EXT[ext];
        if (!mime)
            continue;
        const data = media.getData();
        if (!(data === null || data === void 0 ? void 0 : data.length) || data.length > 12 * 1024 * 1024)
            continue;
        const pct = emuToPct(rect, slideCx, slideCy);
        if (pct.w < 0.5 || pct.h < 0.5)
            continue;
        boxes.push({
            kind: 'image',
            ...pct,
    ...
    for (const frame of extractTopLevelBlocks(flat, 'p:graphicFrame')) {
        ...
        if (pct.w < 0.5 || pct.h < 0.5)
            continue;
        const tbl = (_b = frame.match(/<a:tbl[\s>][\s\S]*?<\/a:tbl>/i)) === null || _b === void 0 ? void 0 : _b[0];
        if (!tbl)
            continue;
    ...
    for (const sp of extractTopLevelBlocks(flat, 'p:sp')) {
        ...
        rect = applyGroupTransform(rect, group);
        const pct = emuToPct(rect, slideCx, slideCy);
        if (pct.w < 0.4 || pct.h < 0.4)
            continue;
        const spPr = ((_c = sp.match(/<p:spPr[\s>][\s\S]*?<\/p:spPr>/i)) === null || _c === void 0 ? void 0 : _c[0]) || '';
        ...
        const txBody = (_d = sp.match(/<p:txBody[\s>][\s\S]*?<\/p:txBody>/i)) === null || _d === void 0 ? void 0 : _d[0];
        ...
        if (txBody) {
            ...
            const { html, plainLines, bold, color, fontSizePt } = paragraphsToHtml(txBody, theme, fallbackPt);
        ...
        if (fill && shapeLooksLikeBox(sp, fill)) {
            if (pct.w > 95 && pct.h > 95)
                continue; // Folienhintergrund separat
            boxes.push({
                kind: 'shape',
                ...pct,
                fillColor: fill,
 ...
 */
function parsePptxBuffer(buffer, fileName = 'import.pptx') {
    ...
    for (let i = 0; i < slideEntries.length; i++) {
        ...
        const boxes = [];
        extractShapeBoxes(slideXml, zip, relMap, placeholders, null, slideCx, slideCy, theme, boxes);
        const cleanedBoxes = dedupeTinyOverlaps(boxes);
        const textBoxes = cleanedBoxes.filter((b) => b.kind === 'text');
        let title = '';
        if (textBoxes.length) {
        ...
        const bodyLines = textBoxes.slice(title ? 1 : 0).flatMap((b) => b.html
            .replace(/<\/(p|li)>/gi, '\n')
            .replace(/<[^>]+>/g, '')
            .split(/\n+/)
            .map((l) => l.trim())
...
//# sourceMappingURL=pptxParse.js.map
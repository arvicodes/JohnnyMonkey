...
<body>
  <div class="container">
    ...
    static async readAudioFile(req, res) {
        try {
            ...
            const jmRoot = path_1.default.resolve(storageManager_1.StorageManager.resolveGitInternRelativePath(''));
            const normalizedFull = path_1.default.resolve(fullPath);
            if (normalizedFull !== jmRoot && !normalizedFull.startsWith(jmRoot + path_1.default.sep)) {
                return res.status(403).json({ error: 'Zugriff nicht erlaubt' });
            }
            const ext = path_1.default.extname(normalizedFull).toLowerCase();
            const base = path_1.default.basename(normalizedFull);
            const wantVideo = req.query.kind === 'video' ||
                req.query.video === '1' ||
                /^slide-screen-/i.test(base);
            const mimeByExt = {
                '.webm': wantVideo ? 'video/webm' : 'audio/webm',
                '.m4a': 'audio/mp4',
                '.mp4': wantVideo ? 'video/mp4' : 'audio/mp4',
                '.ogg': 'audio/ogg',
                '.oga': 'audio/ogg',
                '.mp3': 'audio/mpeg',
                '.wav': 'audio/wav',
            ...
            const stat = fs_1.default.statSync(normalizedFull);
            ...
            if (range) {
                ...
                fs_1.default.createReadStream(normalizedFull, { start, end }).pipe(res);
                return;
            }
            res.setHeader('Content-Length', String(size));
            fs_1.default.createReadStream(normalizedFull).pipe(res);
        }
        catch (error) {
...
- Verwende konkrete Beispiele, Zahlen oder Situationen in den Fragen`
    ...
    static async updateSingleQuestion(req, res) {
        try {
            ...
            const taskPattern = new RegExp(`<!-- Aufgabe ${taskNumber}\\s*(?::([^>]*))?\\s*-->([\\s\\S]*?)(?=<!-- Aufgabe |<div class="submit-section">)`, 'i');

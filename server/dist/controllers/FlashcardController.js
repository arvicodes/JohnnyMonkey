...
const getFlashcardAssignments = async (req, res) => {
    try {
...
const assignDeckToGroup = async (req, res) => {
    try {
...
const removeDeckAssignment = async (req, res) => {
    var _a;
    try {
        const { assignmentId } = req.params;
        const { teacherId } = req.body;
...
const createFlashcardDeckFromWord = async (req, res) => {
    try {
        ...
        let parsedDocument;
        try {
            parsedDocument = await (0, flashcardWordParser_1.parseFlashcardWordFile)(sourceFile);
        ...
        const createdCards = await Promise.all(parsedDocument.cards.map((card, index) => prisma.flashcard.create({
            data: {
                deckId: deck.id,
                front: card.front,
                back: card.back,
        ...
        if (learningGroupIds && learningGroupIds.length > 0) {
            ...
            if (validGroups.length > 0) {
                const assignments = await Promise.all(validGroups.map((group) => prisma.flashcardAssignment.create({
                    data: {
                        deckId: deck.id,
                        groupId: group.id
                    }
...
const addFlashcardsToExistingDeck = async (req, res) => {
    try {
...
const getFlashcardDecks = async (req, res) => {
    try {
...
const getDocumentProcessingHistory = async (req, res) => {
    try {
...
const getStudentAssignedFlashcards = async (req, res) => {
    try {
...
const getStudentAllProgress = async (req, res) => {
    try {
        const { studentId } = req.params;
        if (!studentId) {
            return res.status(400).json({ error: 'Schüler-ID ist erforderlich' });
...
const getAllAssignedCards = async (req, res) => {
    try {
        const { studentId } = req.params;
        if (!studentId) {
            return res.status(400).json({ error: 'Schüler-ID ist erforderlich' });
...
const migrateToNewSpacedRepetitionSystem = async (req, res) => {
    var _a;
    try {
...
const markAllDueCardsAsLearned = async (req, res) => {
    try {
...
async function resolveStudentUserFromLoginHeader(req) {
...
const getCollaborativeDeckCards = async (req, res) => {
    try {
        const { deckId } = req.params;
...
exports.getCollaborativeDeckCards = getCollaborativeDeckCards;
/** SuS: Vollständiges Deck wie in der Lehrer-Ansicht (Karten + Metadaten), für gemeinsames Erstellen. */
const getCollaborativeDeckFull = async (req, res) => {
    try {
        const { deckId } = req.params;
        const groupId = typeof req.query.groupId === 'string' ? req.query.groupId : '';
        const student = await resolveStudentUserFromLoginHeader(req);
...
exports.getCollaborativeDeckFull = getCollaborativeDeckFull;
/** SuS: Karte zum gemeinsamen Deck hinzufügen (nur wenn Gruppe das Deck zugewiesen hat). */
const createCollaborativeCard = async (req, res) => {
    var _a;
    try {
        const { deckId } = req.params;
        const student = await resolveStudentUserFromLoginHeader(req);
        ...
        const { groupId, front, back } = req.body;
        ...
        const nextOrder = ((_a = maxRow._max.order) !== null && _a !== void 0 ? _a : 0) + 1;
        const card = await prisma.flashcard.create({
            data: {
                deckId,
                front: tf,
...
exports.createCollaborativeCard = createCollaborativeCard;

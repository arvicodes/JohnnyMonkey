...
exports.parseFlashcardWordFile = parseFlashcardWordFile;
...
function detectAndParseFormat(lines) {
...
function detectTableFormat(lines) {
    ...
    const tableIndicators = lines.filter(line => line.includes('\t') ||
        line.match(/\s{3,}/) ||
        line.includes('|') ||
        line.match(/^.+?\s{2,}.+$/));
...
function detectAlternatingFormat(lines) {
    // Look for patterns like "Frage:", "Antwort:", "Q:", "A:", etc.
    const questionIndicators = lines.filter(line => line.toLowerCase().match(/^(frage|question|q|f):\s*/) ||

...
class StorageManager {
     ...
     */
    static resolveImageFilePath(filePath) {
    ...
    static resolveSameImageNameNearby(filePath) {
        ...
        if (rel.startsWith('git-intern/'))
            rel = rel.slice('git-intern/'.length);
        else if (markerAt >= 0)
            rel = rel.slice(markerAt + marker.length);
        const parts = rel.split('/').filter(Boolean);
        if (parts.length)
            parts.pop();
        for (let depth = parts.length; depth >= 0; depth -= 1) {
            const dirRel = parts.slice(0, depth);
            const dirFull = dirRel.length ? path_1.default.join(jmRoot, ...dirRel) : jmRoot;
            for (const candidate of [
                path_1.default.join(dirFull, basename),
                path_1.default.join(dirFull, 'Grafiken', basename),
            ]) {

...
export function periodTimesToJson(periods: PeriodTime[]): string {
  return JSON.stringify(periods);
}

/** Europe/Berlin wall-clock parts for scheduling */
export function getBerlinNow(): { date: string; dayOfWeek: number; hours: number; minutes: number; now: Date } {
...
export function berlinDateTime(date: string, time: string): Date {
  ...
  for (let offsetHours = 0; offsetHours <= 3; offsetHours++) {
    ...
    if (berlin === target) return candidate;
  }

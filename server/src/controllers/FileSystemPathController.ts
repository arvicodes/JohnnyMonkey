...
<body>
  ...
  static async createExamination(req: Request, res: Response) {
    try {
      ...
      const templateRel = path.join('J-M-Reihen', 'Mathe', '_Vorlagen', 'Pruefung-Standardvorlage.html');
      if (process.env.NODE_ENV === 'production') {
        const serverPath = path.join(process.cwd(), templateRel);
        const projectPath = path.join(process.cwd(), '..', templateRel);
      ...
      if (!fs.existsSync(templatePath)) {
        ...
        if (process.env.NODE_ENV === 'production') {
          const serverPath = path.join(process.cwd(), legacyRel);
          const projectPath = path.join(process.cwd(), '..', legacyRel);
          templatePath = fs.existsSync(serverPath) ? serverPath : projectPath;
        } else {
          templatePath = path.join(
            '/Users/verachrist/Documents/MEINE_APP/JohnnyMonkey',
      ...
      templateContent = templateContent.replace(
        /Hausaufgabenüberprüfung \(HÜ\): NEUER TITEL/g,
        finalTitle
      );
      templateContent = templateContent.replace(
      ...
      templateContent = templateContent.replace(
        /<div class="timer-container" id="timer">\s*[\d:]+|__TIMER__\s*<\/div>/,
        `<div class="timer-container" id="timer">\n            ${timerLabel}\n        </div>`
      );
      templateContent = templateContent.replace(
        /(<span class="aids-val" id="aidsTime"[^>]*>)[^<]*(<\/span>)/,
        `$1${resolvedDurationMinutes} Min$2`
      );
      templateContent = templateContent.replace(
        /let timeLeft = \d+ \* 60;.*$/m,

...
function paragraph(text: string, sizePt: number, bold: boolean): string {
...
function textBox(id: number, name: string, x: number, y: number, cx: number, cy: number, inner: string): string {
  return (
    `<p:sp>` +
...
export type PptxSlide = { title: string; body: string };

export function buildTextPptx(slides: PptxSlide[]): Buffer {

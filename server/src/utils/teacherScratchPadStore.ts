...
function resolveScratchPadDbFile(): string {
  ...
  const candidates = [
    ...
    envFile && path.isAbsolute(envFile) ? envFile : '',
  ...
  return path.join(projectRoot(), 'server/prisma/dev.db');
}

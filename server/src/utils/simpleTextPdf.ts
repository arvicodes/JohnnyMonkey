...
export type PdfBlock = { title?: string; body?: string };

export function buildTextPdf(docTitle: string, blocks: PdfBlock[]): Buffer {
  const maxChars = 88;
  type PageLine = { text: string; size: number; bold: boolean };
  const pages: PageLine[][] = [];
  let current: PageLine[] = [];
  const maxY = PAGE_H - MARGIN;
  let y = PAGE_H - MARGIN;

  const pushLine = (line: PageLine) => {
    if (y - LINE < MARGIN) {
      pages.push(current);
      current = [];
      y = PAGE_H - MARGIN;
  ...
  if (pages.length === 0) pages.push([{ text: docTitle, size: TITLE_SIZE, bold: true }]);

...
export function mssPointsToGermanGrade(mssPoints: number): number {
  if (mssPoints >= 13) return 1.0;
  if (mssPoints >= 10) return 2.0;
  if (mssPoints >= 7) return 3.0;
  if (mssPoints >= 4) return 4.0;
  if (mssPoints >= 1) return 5.0;
  return 6.0;
}
...
export function germanGradeToMSSPoints(germanGrade: number): number {
  if (germanGrade <= 1.7) return 13;
  if (germanGrade <= 2.7) return 10;
  if (germanGrade <= 3.7) return 7;
  if (germanGrade <= 4.7) return 4;
  if (germanGrade <= 5.7) return 1;
  return 0;
}
